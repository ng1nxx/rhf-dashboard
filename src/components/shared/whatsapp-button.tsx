...
/** The WhatsApp glyph — Lucide v1 no longer ships brand icons. */
 ...
 * the number always comes from site settings and the link is always encoded
 * the same way — PRD §18.4.
 */
export function WhatsAppButton({
  ...
  return (
    <Button asChild variant={variant} size={size} className={className} {...props}>
      ...
      >
        {showIcon ? <WhatsAppIcon aria-hidden /> : null}
        {children}
      </a>
    </Button>
  );
